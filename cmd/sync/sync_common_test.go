...
// init устанавливает getClient для тестов (функция будет переопределена в тестах при необходимости)
func init() {
	SetGetClientForTests(func(cmd *cobra.Command) *client.HTTPClient {
		// В тестах используется mockClient через newMigration, поэтому здесь возвращаем nil
		// Реальные HTTP вызовы не выполняются
		return nil
	})
}
