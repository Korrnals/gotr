...
> [!TIP]
> For a quick `groups` workflow: inspect `--help`, then run a
...
```bash
gotr groups add --help && gotr groups add
```
...
- Use `groups` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `groups`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr groups --help`, then inspect the target subcommand help before execution.
  >
  > ---
