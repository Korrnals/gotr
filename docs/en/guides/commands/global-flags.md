...
> [!TIP]
> For a quick `global-flags` workflow: inspect `--help`, then run a
...
- Use `global-flags` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `global-flags`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr global-flags --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: add, attachments, bdds, cases, compare, completion, config, datasets. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
...
- Command completes without errors and returns expected scenario output.
