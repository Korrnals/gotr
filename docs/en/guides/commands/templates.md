...
> [!TIP]
> For a quick `templates` workflow: inspect `--help`, then run a
...
- Use `templates` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `templates`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr templates --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: list. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
