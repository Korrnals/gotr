...
> [!TIP]
> For a quick `update` workflow: inspect `--help`, then run a
...
- Use `update` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `update`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr update --help`, then inspect the target subcommand help before execution.
  >
  > ---
...
- Compare before/after state via `get` and confirm only intended fields changed.
