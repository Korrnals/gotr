...
func TestWrite_AllFormats(t *testing.T) {
	...
	for _, p := range out.Files() {
		...
		// Entity types in fixedReport are ["result", "case"] → subdir "group-entity".
		if !strings.Contains(p, filepath.Join("cleanup-attachments", "group-entity", "audit-2026-05", "2026-05-05")) {
