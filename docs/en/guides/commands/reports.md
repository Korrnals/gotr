...
> [!TIP]
> For a quick `reports` workflow: inspect `--help`, then run a
...
| `list-cross-project` | Получить список кросс-проектных отчётов |
...
```bash
gotr reports list --help && gotr reports list
```
...
- Use `reports` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr reports --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: list, run, run-cross-project. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
