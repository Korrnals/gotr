...
> [!TIP]
> For a quick `tests` workflow: inspect `--help`, then run a
...
```bash
gotr tests get --help && gotr tests get
```
...
- Use `tests` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `tests`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr tests --help`, then inspect the target subcommand help before execution.
  >
  > ---
