...
> [!TIP]
> For a quick `get` workflow: inspect `--help`, then run a
...
```bash
gotr get case --help && gotr get case
```
...
- Use `get` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `get`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr get --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: case, case-fields, case-history, case-types, cases, project, projects, sharedstep. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
...
- Use repeated `get` calls as post-check after mutate operations.
