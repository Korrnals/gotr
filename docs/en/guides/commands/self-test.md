...
> [!TIP]
> For a quick `self-test` workflow: inspect `--help`, then run a
...
- Use `self-test` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `self-test`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr self-test --help`, then inspect the target subcommand help before execution.
  >
  > ---
...
- Critical checks pass and command exits with code `0`.
- In `--failures-only` mode, no failing checks remain.

### Output shape example (`--json`)
...
Note: check names/count depend on runtime environment and project state.
...
- If failures appear, fix environment issues and rerun until stable `ok` state.
