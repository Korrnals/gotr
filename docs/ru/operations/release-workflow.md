...
Language: Русский | [English](../../en/operations/release-workflow.md)
...
- [ ] Версия в `cmd/root.go` (`Version`) поднята до `X.Y.Z` (без `-dev`-суффикса)
- [ ] **Бейдж в README обновлён** в обоих файлах (`README.md` и `README_ru.md`): `release-vX.Y.Z`
- [ ] PR: release → main
- [ ] Tag `vX.Y.Z`

> **Принцип версионирования.** `main` всегда отражает последний выпущенный тег: константа `Version` в `cmd/root.go` совпадает с актуальным `vX.Y.Z`, бейдж в README — тоже. `-dev`-суффиксы и «next-версии» в README не используем: релизные бинарники получают версию из git-тега через `-ldflags`, а планы по следующему релизу живут в `[Unreleased]`-секции CHANGELOG, а не в шапке README. Поднятие `Version` до следующего номера происходит **в release-ветке** при подготовке релиза, на `main` приезжает merge-коммитом релизного PR.

> **Зачем синхронизировать бейдж.** Бейдж в шапке README — публичное лицо репозитория. Устаревший `release-vX.Y` после выпуска нового тега выглядит как заброшенный проект. Считаем его частью релизного артефакта, а не декорацией.
...
← [Эксплуатация](index.md) · [Документация](../index.md)
