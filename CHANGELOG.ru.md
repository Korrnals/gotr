...
- **`gotr attachments cleanup` теперь пишет аудит-отчёт.** После каждого
  запуска (включая `--dry-run`) команда сохраняет отчёт об удалении в
  четырёх форматах — Markdown, JSON, CSV и PDF — под
  `~/.gotr/reports/cleanup-attachments/<label>/<YYYY-MM>/`. В отчёте
  присутствуют заголовок (snapshot ID, label, CLI-аргументы, версия
  gotr), применённые фильтры, сводка, разбивка по проектам, полный
  список удалённых вложений (id, имя, размер, родитель, дата создания)
  и список ошибок. Markdown содержит команду отката, `INDEX.md`
  обновляется автоматически.
- **`--no-report`** отключает запись (полезно для CI, который ловит
  только stdout).
- **Отчёты dry-run** содержат маркер `DRY-RUN` в шапке Markdown и
  `dry_run=true,deleted=false` в строках CSV — их нельзя спутать с
  реальным артефактом.
- **Обратная совместимость.** Изменение поведения — четыре дополнительных
  файла на запуск; флаги и пути остальных артефактов не менялись.

### Internal

- Новый пакет `internal/report/cleanup` (типы + Markdown / JSON / CSV
  рендеры + writer) и `pdf.NewCleanupGenerator()`, переиспользующий
  встроенные шрифты NotoSans.
- Категория `cleanup-attachments` добавлена в
  `internal/report/paths.go` и `internal/retention/exports.go`
  (поддержка расширения `.csv`).
...
Патч-релиз, объединяющий два хотфикса.
