...
Команда `gotr delete` удаляет ресурсы через API TestRail.

## Что делает

- Удаляет существующие сущности по ID.
- Подходит для очистки временных/ошибочных объектов после прогонов.
- Помогает поддерживать порядок в тестовых проектах при массовых операциях.
