...
- [x] Pipeline-параллелизация загрузки кейсов (`internal/parallel`)
- [x] Централизованный reporter для compare-команд (`internal/ui/reporter`)
...
### compare * (11 подкоманд) → `internal/ui/reporter/` (builder pattern)
Централизованный вывод статистики через `reporter.New().Section().Stat().Print()`.
...
- `internal/parallel` - Pipeline pagination controller (compare cases)
- `internal/ui` - Unified display layer (ANSI live display + reporter)
