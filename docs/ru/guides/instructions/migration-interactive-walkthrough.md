...
### Частый вопрос про «свободные» ID в target

- Shared steps не копируются с исходными ID.
- Даже если в target нет объекта с таким номером, этот ID не назначается принудительно.
- Новый ID всегда выдается TestRail при создании shared step.
- Стабильность связей обеспечивается не совпадением номеров, а remap-ом через mapping.
