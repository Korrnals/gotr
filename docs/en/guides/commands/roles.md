...
Language: [Русский](../../../ru/guides/commands/roles.md) | English
...
## Overview 🎯
Manage TestRail user roles.
Roles define the set of access permissions for users in the TestRail system.

> [!TIP]
> For a quick `roles` workflow: inspect `--help`, then run a
...
```bash
gotr roles get --help && gotr roles get
```
...
- Use `roles` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `roles`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr roles --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: get, list. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
