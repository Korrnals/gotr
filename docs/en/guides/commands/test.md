...
Language: [Русский](../../../ru/guides/commands/test.md) | English
...
## Overview 🎯
Commands for retrieving and managing tests in TestRail.
A test is a specific instance of a test case within a test run.

> [!TIP]
> For a quick `test` workflow: inspect `--help`, then run a
...
```bash
gotr test get --help && gotr test get
```
...
- Use `test` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `test`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr test --help`, then inspect the target subcommand help before execution.
  >
  > ---
