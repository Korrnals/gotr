...
> [!TIP]
> For a quick `cases` workflow: inspect `--help`, then run a
...
- Use `cases` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `cases`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr cases --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: add, bulk, delete, get, list, update. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
