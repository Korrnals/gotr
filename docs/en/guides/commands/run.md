...
## Overview 🎯
Commands for managing test runs in TestRail.
A test run is an instance of a test suite launched for executing tests.

> [!TIP]
> For a quick `run` workflow: inspect `--help`, then run a
...
| --- | --- |
| `close` | Close a test run |
| `create` | Create a new test run |
| `delete` | Delete a test run |
| `get` | Get test run information |
| `list` | List project test runs |
| `update` | Update a test run |
...
```bash
gotr run close --help && gotr run close
```
...
- Use `run` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `run`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr run --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: close, create, delete, get, list, update. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
