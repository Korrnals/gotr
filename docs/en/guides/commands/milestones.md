...
> [!TIP]
> For a quick `milestones` workflow: inspect `--help`, then run a
...
```bash
gotr milestones add --help && gotr milestones add
```
...
- Use `milestones` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `milestones`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr milestones --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: add, delete, get, list, update. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
