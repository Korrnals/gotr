...
| `rollback list` | Просмотр только откаченных снапшотов (интерактивный браузер) |
| `rollback undo [id]` | Отмена отката — удаление пересозданных сущностей |
...
## Навигационный хаб (`gotr work`) 🧭

Snap доступен через интерактивный навигационный хаб:

```bash
gotr work
```

В главном меню выберите **📦 Snap — snapshots and rollback**, затем — нужную подкоманду.
После выполнения операции в snap post-action меню доступна кросс-навигация:

- **📊 Compare** — перейти к сравнению проектов
- **🔄 Sync** — перейти к миграции
- **📦 Snap** — список снапшотов

Аналогично, из post-action меню Compare и Sync доступен переход в Snap.

> В интерактивном режиме при подключении к серверу отображается баннер:
...
- ❓ **Вопрос:** Можно ли откатить дважды?
  > ↪️ **Ответ:** Нет. После отката статус снапшота меняется на `rolled_back`, повторный откат блокируется. Однако можно **отменить откат** через `gotr snap rollback undo <id>` — это удалит пересозданные сущности.
