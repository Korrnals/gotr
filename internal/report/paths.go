...
// should live: <Category>[/p<Project>]/<Label>/<Date>/.
...
func (c Classification) RelDir() string {
	...
	default:
		...
		return filepath.Join(string(c.Category), label, c.Date)
	}
}
...
func ClassifyReportWithLabel(filename, explicitLabel string) Classification {
	...
	case strings.HasPrefix(low, "migration-"):
		return Classification{Category: CategoryMigrations, Label: label, Date: date}

	case strings.HasPrefix(low, "rollback-"):
		return Classification{Category: CategoryRollbacks, Label: label, Date: date}

	case strings.HasPrefix(low, "cleanup-attachments-"):
		return Classification{Category: CategoryCleanupAttachments, Label: label, Date: date}
	...
	case strings.HasPrefix(low, "gotr_migration_"):
		...
		return Classification{Category: CategoryCoverage, Label: label, Date: date}
