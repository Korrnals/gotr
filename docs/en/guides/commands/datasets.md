...
> [!TIP]
> For a quick `datasets` workflow: inspect `--help`, then run a
...
```bash
gotr datasets add --help && gotr datasets add
```
...
- Use `datasets` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `datasets`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr datasets --help`, then inspect the target subcommand help before execution.
  >
  > ---
