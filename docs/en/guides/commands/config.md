...
Language: [Русский](../../../ru/guides/commands/config.md) | English
...
## Overview 🎯
Commands for managing the gotr configuration file.

> [!TIP]
> For a quick `config` workflow: inspect `--help`, then run a
...
| --- | --- |
| `edit` | Open config file in the default editor |
| `init` | Create default configuration file |
| `path` | Show path to current config file |
| `view` | Show contents of current config file |
...
- Use `config` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `config`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr config --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: edit, init, path, view. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
...
- `view/path/edit/init` commands execute cleanly and reflect active configuration state.
- After `init`, a valid config file exists with expected keys.
...
- After config changes, run a read command (`get/list`) as smoke check.
