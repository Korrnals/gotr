...
> [!TIP]
> For a quick `delete` workflow: inspect `--help`, then run a
...
- Use `delete` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `delete`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr delete --help`, then inspect the target subcommand help before execution.
  >
  > ---
...
- After deletion, verify resource is no longer returned by `get/list`.

```bash
# delete
gotr delete <endpoint> <id>

# validate via list/read
gotr list <resource>
```
