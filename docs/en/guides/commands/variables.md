...
Language: [Русский](../../../ru/guides/commands/variables.md) | English
...
## Overview 🎯
Manage variables — configuration values
for test cases.

> [!TIP]
> For a quick `variables` workflow: inspect `--help`, then run a
> safe/validation command before operational execution.

## Syntax 🧩
```bash
gotr variables [command]
```
...
| --- | --- |
| `add` | Create a variable in a dataset |
...
```bash
gotr variables add --help && gotr variables add
```
...
- Use `variables` when the task belongs to this command domain and you need predictable repeatable behavior.
...
- ❓ **Question:** When should I use `variables`?
...
- ❓ **Question:** Where should I start if parameters are unclear?
  > ↪️ **Answer:** always start with `gotr variables --help`, then inspect the target subcommand help before execution.
  >
  > ---

- ❓ **Question:** Which subcommands should be validated first?
  > ↪️ **Answer:** recommended starting set: add, delete, list, update. Begin with the highest-frequency operation in your release workflow.
  >
  > ---
