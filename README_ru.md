# gotr — CLI-утилита для TestRail API

[English](README.md) | [Русский](README_ru.md)

`gotr` — мощный и удобный инструмент командной строки для работы с TestRail API v2.  
Позволяет выполнять GET-запросы, экспортировать данные в файлы, фильтровать ответы через встроенный `jq` и многое другое — без необходимости устанавливать внешние зависимости.

## Особенности

- Полная поддержка GET-эндпоинтов TestRail API
- Встроенный `jq` — фильтрация без установки внешней утилиты
- Экспорт данных в JSON-файлы (автоматическое именование или через `--output`)
- Автодополнение ресурсов и эндпоинтов
- Гибкие флаги: `--quiet`, `--type`, `--jq`, `--project-id` и другие
- Поддержка query-параметров (suite_id, section_id и т.д.)
- Полностью автономный бинарник — работает везде, где есть Go

## Установка

### Скачать готовый бинарник одной командой (Linux/macOS)

```bash
curl -L https://github.com/Korrnals/gotr/releases/latest/download/gotr-$$ (uname -s)- $$(uname -m) -o gotr && \
chmod +x gotr && \
sudo mv gotr /usr/local/bin/
```

> [!TIP] Примечание
>
> Замени ***latest*** на конкретную версию, если нужно (например, ***v1.0.0***).\
> Для Windows — скачай .exe вручную из Releases.
>
> ***Бинарники для Linux, macOS и Windows будут доступны в [Releases](https://github.com/Korrnals/gotr/releases).***

### Вариант 1: Из исходников (рекомендуется)

```bash
# Клонируем репозиторий
git clone https://github.com/Korrnals/gotr.git
cd gotr

# Собираем бинарник (оптимизированный и сжатый)
go build -ldflags="-s -w" -o gotr

# (Опционально) Сжимаем ещё сильнее с помощью UPX
upx --best gotr

# Перемещаем в PATH
sudo mv gotr /usr/local/bin/
```

### Вариант 2: Установка через Makefile (рекомендуется для разработчиков)

**Makefile** упрощает сборку, тестирование и установку.

```bash
# Клонируем репозиторий
git clone https://github.com/Korrnals/gotr.git
cd gotr

# Сборка и установка одной командой
make install

# Другие полезные команды:
make build              # только собрать
make test               # запустить тесты
make compress           # сжать UPX (если установлен)
make build-compressed   # собрать + сжать
make clean              # очистить
make release            # собрать для всех платформ

make install            # соберёт оптимизированный бинарник и установит в /usr/local/bin (требует sudo).

# Если установлен UPX — используй 
make build-compressed   # для минимального размера (~3-5 МБ).

# Пример сжатия UPX для Windows пакета
make compress BINARY_NAME=gotr.exe
```

**Сборка с версией:**

```bash
# Нет тега — версия "dev"
make build
# gotr version - dev

# Делаешь тег
git tag v1.0.0
make build
# gotr version - v1.0.0

# Явно указываешь версию — приоритет выше тега
make build VERSION=test-123
# gotr version - test-123
```

---

### Установка в Windows

Для Windows:

- Нет sudo, установка в PATH вручную.
- Бинарник с расширением .exe.
- UPX работает на Windows.
- Curl однострочник — чуть другой (PowerShell или cmd).

#### Вариант 1: Скачать готовый бинарник одной командой (PowerShell)

```powershell
Invoke-WebRequest -Uri https://github.com/Korrnals/gotr/releases/latest/download/gotr-windows-amd64.exe -OutFile gotr.exe
# Делаем исполняемым (не обязательно на Windows, но для безопасности)
# Перемещаем в директорию из PATH (например, C:\Windows или пользовательский bin)
Move-Item gotr.exe C:\Windows\gotr.exe
```

#### Вариант 2: Из исходников

```powershell
git clone https://github.com/Korrnals/gotr.git
cd gotr
go build -ldflags="-s -w" -o gotr.exe

# (Опционально) Сжатие UPX
upx --best gotr.exe

# Перемещаем в PATH
Move-Item gotr.exe C:\Windows\
```

#### Вариант 3: Через Makefile (требует Make для Windows, например Chocolatey: choco install make)

```powershell
git clone https://github.com/Korrnals/gotr.git
cd gotr
make build          # собрать gotr.exe
make compress       # сжать UPX (если установлен)
# Установка вручную:
Copy-Item gotr.exe C:\Windows\
```

> [!TIP] Примечание
> На **Windows** рекомендуется добавить директорию в **PATH** через "***Параметры → Система → О системе → Дополнительные параметры системы → Переменные среды***".

## Конфигурация

`gotr` поддерживает несколько способов аутентификации:

### Через флаги

```bash
gotr --base-url https://your-company.testrail.io/ \
     --username your@email.com \
     --api-key your_api_key \
     get get_projects
```

### Через переменные окружения

```bash
export TESTRAIL_BASE_URL="https://your-company.testrail.io/"
export TESTRAIL_USERNAME="your@email.com"
export TESTRAIL_API_KEY="your_api_key"

gotr get get_projects
```

### Через конфиг-файл (скоро)

---
---

## Использование

### Основные команды

```bash
gotr get <endpoint> [id]                # GET-запрос
gotr export <resource> <endpoint> [id]  # Экспорт в файл
gotr list <resource>                    # Список доступных эндпоинтов
```

### Примеры

#### Получить список проектов

```bash
gotr get get_projects
gotr get get_projects -t table          # в табличном виде
gotr get get_projects -j                # с встроенным jq (форматирование)
gotr get get_projects -j -f '.[] .name' # только имена проектов
```

#### Получить проект по ID

```bash
gotr get get_project 30
gotr get get_project --project-id 30    # через флаг
gotr get get_project 30 -o project30.json  # сохранить в файл
```

#### Получить кейсы с фильтрацией

```bash
gotr get get_cases 30 --suite-id 20069
gotr get get_cases 30 --suite-id 20069 --section-id 10
gotr get get_cases --project-id 30 --suite-id 20069
```

#### Экспорт данных

```bash
gotr export cases get_cases 30 --suite-id 20069
# Файл сохранится в .testrail/cases_30_*.json

gotr export cases get_cases 30 --suite-id 20069 -o my_cases.json
# Сохранить в указанный файл
```

#### Автодополнение

`gotr` поддерживает автодополнение ресурсов и эндпоинтов:

```bash
gotr get <Tab><Tab>        # предложит эндпоинты
gotr export cases <Tab>    # предложит эндпоинты для cases
```

---
---

## Флаги

### Глобальные

- `--base-url` — базовый URL TestRail
- `--username` / `-u` — email пользователя
- `--api-key` / `-k` — API-ключ
- `--config` / `-c` — путь к конфиг-файлу
- `--insecure` / `-i` — пропустить проверку TLS
- `--jq` / `-j` — вывод через встроенный jq
- `--jq-filter` / `-f` — фильтр jq
- `--quiet` / `-q` — подавить вывод на экран
- `--type` / `-t` — формат вывода (json, json-full, table)
- `--output` / `-o` — сохранить в файл

### Локальные (для get/export)

- `--project-id` / `-p` — ID проекта
- `--suite-id` / `-s` — ID тест-сюиты
- `--section-id` — ID секции
- `--milestone-id` — ID milestone

## Лицензия

MIT License — используй, модифицируй, распространяй свободно.

## Авторы

- [Korrnals](https://github.com/Korrnals)

## Благодарности

- TestRail API
- jqlang/jq - отличный инструмент работы с JSON
- itchyny/gojq — вдохновение для встроенного jq
- spf13/cobra — основа CLI
- fatih/color — цветной вывод

---

⭐ Если утилита полезна — поставь звёздочку на GitHub!  
Есть идеи или баги — открывай issue или PR.

Спасибо, что используешь `gotr`! 🚀
